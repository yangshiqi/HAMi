...
具体参考每个类型配置（chipName）中的 aiCore 和 template 下的 aiCore 配比。
...
Ascend310P 设备（Atlas 推理系列产品）支持多种粒度的切分，包括 1/7 卡、1/4 卡和 1/2 卡，分配的显存会自动对齐到在分配额之上最近的粒度上。
...
在节点注解中，查找 `hami.io/node-register-<card-type>` 或类似的注解，其中包含设备 UUID 信息。
